...
export default function EmailForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setEmail("");
        toast.success("Obrigado por entrar na nossa lista de espera! 🚀");
      } else {
        setEmail("");
  ...
  return (
    <>
      ...
      <div className="flex items-start gap-2 text-gray-500">
        ...
        Não se preocupe! Seus dados estão completamente seguros e serão utilizados apenas para fornecer atualizações sobre nosso produto.
        </p>
      </div>
    </>
  );
